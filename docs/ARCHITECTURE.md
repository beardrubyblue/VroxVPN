...
### Статистика трафика в UI (паритет с Python core/stats.py)

Python-версия читала rx/tx байты прямо с TUN-интерфейса
(`/proc/net/dev`, см. `core/stats.py` в ветке main) — Linux-специфичный
подход, который напрямую не переносится на macOS NE (нет настоящего
интерфейса с именем, который можно было бы так прочитать). Сделано
по-разному на двух платформах, с одинаковым контрактом наружу:

- **Linux** (`engine/linux.rs::get_traffic_totals`) — тот же
  `/proc/net/dev` на `tun-vroxory`, без изменений в подходе, просто
  перенесено из Python в Rust.
- **macOS** (`netunnel.go::TunnelHandle.{tx,rx}Bytes` + `GetStats()` →
  `PacketTunnelProvider.swift::handleAppMessage` → `engine/macos.rs::
  get_traffic_totals` через `NETunnelProviderSession.sendProviderMessage`) —
  счётчики живут в Go, на границе `WritePacket`/`ReadPacket`
  (единственная точка, через которую проходит вообще весь трафик
  тоннеля в обе стороны, независимо от TCP/UDP — тот же принцип, что у
  /proc/net/dev: считать на границе интерфейса, не на отдельных
  соединениях). Rust опрашивает по запросу (раз в секунду, инициатива
  на стороне фронтенда), а не получает push-уведомления — `.appex` не
  имеет собственного таймера для этого.

Оба пути отдают только суммарные байты с начала тоннеля (не
скорость) — дельту/скорость считает сам фронтенд (`App.tsx`) между
